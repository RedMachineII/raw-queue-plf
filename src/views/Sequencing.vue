          ...
          <b-col class="col-12 col-sm-6 col-md-4 col-lg-3">
            <b-form inline>
              ...
              <b-form-input 
                ...
                type="date" 
              ></b-form-input>
              <b-button variant="transparent" class="btn-new ml-2" @click="getData()">
                <font-awesome-icon :icon="['fas', 'check']" />
                <span class="m-1">ยืนยัน</span>
              </b-button>
            </b-form>
          </b-col>
        </b-row>
        <b-row class="row p-1 justify-content-around">
          <b-col class="col-12">
...
<script>
  ...
  export default {
    ...
    mounted() {
      let format = new Date().getTime() + 86400000;
      format = new Date(format);
      this.plan_date = format.getFullYear() + '-' + ('0' + (parseInt(format.getMonth()) + 1)).slice(-2) + '-' + ('0' + format.getDate()).slice(-2);
      this.getData()
    },
    methods: {
      processData(process) {
